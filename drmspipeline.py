from SDOprocessing import pipeutils, SDOproc, drmsDownload
import sunpy.map
import glob
...
if len(p_hmi_bad) > 0:
	...
	while len(p_hmi_bad) > 0 and retries < rep_tol:
		print(f'Attempt - {retries + 1}') 
		p_hmi_dls, p_hmi_ind, p_hmi_bad = drmsDownload.drms_sdo_patcher(new_start, new_end, p_hmi_bad, path_hmi, wavelengths, path_aia)
		print(p_hmi_bad)
...
for wvl in wavelengths:
	...
	aia_save = f'/Users/danielgass/Desktop/ARCCnetDan/ARCCnet/data_generation/test_files/proc/aia/'
	for map in aia_maps:
