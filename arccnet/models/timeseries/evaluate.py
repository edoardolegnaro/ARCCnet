...
def compute_multiclass_metrics(y_true, probs, threshold=0.5, class_names=None):
    ...
    layout = _get_operational_class_layout(num_classes, class_names=class_names)
    ...
    for class_idx, class_name in enumerate(layout["class_names"]):
        class_key = _normalize_class_name(class_name)
    ...
    for prefix, skill in [("m_plus", m_plus_skill)]:
...
def main(args):
    ...
    if args.save_predictions:
        ...
        if task_type == "multiclass":
            ...
            configured_class_names = list(getattr(ts_config, "FLARE_CLASS_NAMES", []))
            layout = _get_operational_class_layout(num_classes, class_names=configured_class_names)
            x_index = layout["x_index"]
            m_plus_indices = layout["m_plus_indices"]
